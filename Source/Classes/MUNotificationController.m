...
+ (MUNotificationController *) sharedController {
    ...
    static dispatch_once_t token;  //dispatch_once_t token;
    // dispatch_once(&token, ^ はコードを一度だけ実行
    dispatch_once(&token, ^{
...
// キーボードが表示された後にkeyboardDidShowメソッド、キーボードが非表示になった後にkeyboardDidHideメソッドを呼び出す
- (id) init {
    if ((self = [super init])) {
